...
box <- ggplot(plot_data, aes(x = factor(format(DATE, "%Y")), y = UMCSENT)) +
  ...
  geom_point(aes(y = oil_price * scale_factor), color = "darkgreen", alpha = 0.6, size = 2, position = position_jitter(width = 0.2)) +
  geom_smooth(aes(y = oil_price * scale_factor, group = 1), method = "loess", color = "darkgreen", linetype = "solid", size = 1, se = FALSE) +
  labs(
...
ggplot(plot_data, aes(x = factor(format(DATE, "%Y")), y = UMCSENT)) +
...
```{r}
# Rescale S&P500 to match the range of Consumer Sentiment (UMCSENT)
scale_factor_500 <- max(plot_data$UMCSENT, na.rm = TRUE) / max(plot_data$`S&P500`, na.rm = TRUE)

ggplot(plot_data, aes(x = DATE)) +
  geom_line(aes(y = `S&P500` * scale_factor_500, color = "Stock Market (S&P500)"), size = 1.2) +  # Replacing UMCSENT with S&P500
  geom_line(aes(y = MICH * scale_factor, color = "Inflation"), size = 1.2) +
  annotate("rect", xmin = war_start, xmax = war_end, ymin = -Inf, ymax = Inf, fill = "gray80", alpha = 0.5) +
  annotate("text", x = as.Date("2023-02-24"), y = 85, label = "Conflict Period", 
           color = "black", angle = 90, vjust = -0.5, hjust = 1, font = 2) +
  scale_y_continuous(limits = c(0, 150), breaks = seq(0, 150, by = 20)) +
  scale_x_date(limits = c(as.Date("2015-01-01"), max(plot_data$DATE, na.rm = TRUE)),
               date_labels = "%Y",
               date_breaks = "1 year") +
  scale_color_manual(values = c("Stock Market (S&P500)" = "cyan1", "Inflation" = "mediumvioletred")) +
  labs(title = "Stock Market (S&P500) and Inflation Trends During Russia-Ukraine War",
       x = "Date",
